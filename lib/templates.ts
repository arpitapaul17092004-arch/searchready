...
export function generateTemplate(rawTopic: string): ContentTemplate {
  const topic = rawTopic.trim().replace(/\s+/g, " ").slice(0, 120);
  const t = capitalize(topic);

  const metaDescription = `${t}: what it is, how it works, why it matters, and how to get started — a practical, up-to-date guide with answers to common questions.`.slice(
    0,
    158,
  );
  ...
  const sections: TemplateSection[] = [
    {
      ...
      directAnswerPrompt:
        "40–80 words defining the topic in plain language. No history, no fluff — just the definition and one differentiating fact.",
      bulletPoints: [
    ...
    },
    {
      ...
      directAnswerPrompt:
        "A 40–80 word summary of the mechanism, then a numbered step-by-step breakdown.",
      bulletPoints: [
        "3–6 numbered steps",
    ...
    },
    {
      ...
      directAnswerPrompt:
        "One-sentence verdict on when to choose it, followed by a comparison table.",
      bulletPoints: [
  ...
  return {
    ...
    metaDescription,
    introAnswerBlock:
      "Write your 40–80 word direct answer here. It must stand alone: a reader who sees only this paragraph should understand the topic. AI engines (ChatGPT, Perplexity, AI Overviews) overwhelmingly quote self-contained short answers near the top of the page.",
    sections,
    faq,
    keyEntities: [
